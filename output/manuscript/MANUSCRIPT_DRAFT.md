...
**Draft status**: PARTIAL (see DRAFT_README.md). Map-derived numbers come from the 2024 national re-run at 9 km with the adopted nine-index classifier (2026-09-10, output/NATIONAL_2024_9KM_RESULTS.md). The remaining XXX markers are regional (100 km) results awaiting a regional re-run with the adopted classifier, figures to regenerate, and the multi-year template sections. [CHECK] markers flag claims the re-run changed.

<!-- Assembled 2026-09-11 from sections/ by assemble.py. Edit the section files, not this one. -->
...
| ABS sown-area statistics | Independent validation reference | SA2-level, 151 SA2s, national | Public |
...
## 5.8 National multi-year map (2017-2025) [TEMPLATE]

Table 6 summarises the nine national seasons. Classified share ranged from XXX% (XXX) to XXX% (XXX). The two weakest seasons were XXX and XXX, [consistent with / unlike] the regional result in Section 5.6.

**Table 6.** National map by season. ABS SA2 figures exist for 2022-2024 only, so the ABS columns are blank for other seasons. ABARES errors exclude forecast-flagged figures.

| season | polygons | classified (%) | classified area (M ha) | canola / cereal / legume share (%) | ABS canola r | ABS over-call | ABARES state MAE (%) |
|---|---:|---:|---:|---|---:|---:|---:|
| 2017 | XXX | XXX | XXX | XXX / XXX / XXX | - | - | XXX |
| 2018 | XXX | XXX | XXX | XXX / XXX / XXX | - | - | XXX |
| 2019 | XXX | XXX | XXX | XXX / XXX / XXX | - | - | XXX |
| 2020 | XXX | XXX | XXX | XXX / XXX / XXX | - | - | XXX |
| 2021 | XXX | XXX | XXX | XXX / XXX / XXX | - | - | XXX |
| 2022 | XXX | XXX | XXX | XXX / XXX / XXX | XXX | XXXx | XXX |
| 2023 | XXX | XXX | XXX | XXX / XXX / XXX | XXX | XXXx | XXX |
| 2024 | XXX | XXX | XXX | XXX / XXX / XXX | XXX | XXXx | XXX |
| 2025 | XXX | XXX | XXX | XXX / XXX / XXX | - | - | XXX |

Against ABS at SA2, mapped canola composition gave r = XXX, XXX and XXX for 2022, 2023 and 2024. The area over-call was XXXx, XXXx and XXXx for the same seasons, with the excess again in cereal and legume rather than canola [confirm]. Against ABARES state totals, the median absolute error of mapped canola area was XXX% (n = XXX state-seasons). Mapped cereal and legume state totals exceeded ABARES by a median XXXx and XXXx.

Nationally, the median best-match IoU between consecutive seasons was XXX. A polygon was found in a median of XXX of the nine seasons. Stability peaked in the XXX ha range, as it did regionally (Section 5.6).

Of polygons classified in two consecutive seasons and matched at an IoU of at least 0.5, XXX% changed class. The most common sequence was XXX followed by XXX (XXX% of matched pairs). Canola was followed by cereal in XXX% of cases and by canola in XXX% (Fig. 9 [TEMPLATE: national classified share and class area by season, with the ABS and ABARES series overlaid]).
...
The central result of this paper is that the map's main disagreement with ABS statistics is a presence-detection problem rather than a species-classification problem. Three lines of evidence support this reading. First, canola composition matches ABS closely (r = 0.77) while total mapped area over-calls ABS by 1.18x, and dividing out the over-call brings the canola share to within 2.4 points of its expected value (Section 5.2). A presence problem produces this pattern (too much land called crop-present, with the crop mix among that land roughly right); a classification problem would produce the right amount of land called crop with the wrong species assigned to some of it. Second, the same over-call magnitude and the same class concentration (cereal and legume rather than canola) reproduce at national and 100 km regional scale, which an artefact of one region or one year would not do [CHECK: the national over-call is 1.18x on the adopted map against 1.59x on the earlier regional run, so "the same magnitude" needs the regional re-run]. Third, the WorldCereal and NLUM comparisons (Section 5.5), which share no data with the ABS validation, point the same way: presence agreement against WorldCereal is moderate in both directions (80.9% of abstained points and 66.8% of classified points agree) [CHECK: no longer symmetric as on the first map (73.0% and 73.2%), so the argument that agreement is not poor in one direction needs revisiting], and NLUM's grazing-probability layer shows abstained land is more grazing-like than classified land (median grazing probability 57.4% against 17.7%). Non-cropped land, rather than crops the classifier failed to recognise, is therefore the main source of the abstained population.
...
Every polygon in the national map carries either a predicted class or one of four abstain reasons, plus its NDVI amplitude, classification confidence and `no_crop_shape` flag, so a user who needs area-accurate rather than composition-accurate output can filter to a tighter view of the map without waiting for a methodological fix. No population of land known not to be cropped was available to train an explicit non-crop class (Section 4.4), so instead of forcing a binary crop/non-crop decision the available labels could not support, the map reports its uncertainty on every polygon.
...
Legume classification carries its own unresolved error, separate from the presence problem: the national map assigns 16.1% of classified area to legume against an ABS share of 8.9% (Section 5.3). This is not an artefact of the decision rule. Argmax and mean predicted probability give the same share (18.1% and 18.3%; Section 4.6), so the classifier favours legume at this rate rather than reporting it inconsistently. The confusion matrices point the same way but do not account for the size of the gap: canola and cereal paddocks are misclassified as legume more often than the reverse (19 of 155 true canola and 10 of 279 true cereal paddocks under temporal transfer, against 6 and 6 legume paddocks the other way; Section 5.1). This is a species-level confusion (which crop, given that something is present) rather than a presence-level one, and the largest unresolved classification error in the map. We leave the mechanism open (Section 7, Limitation 2).

---

## 6.8 What nine national seasons add [TEMPLATE]

The presence reading of Section 6.1 rests on one national season. The three seasons with ABS SA2 data test whether it depends on that season. The over-call was XXXx, XXXx and XXXx in 2022, 2023 and 2024, with the same class signature [in each / in XXX of them]. [If it holds:] The presence problem is therefore a property of the pipeline rather than of the 2024 season. [If it does not:] The size of the over-call therefore varies with the season, and Section 6.4's filters need a per-season check.

The weak regional seasons (2017 and 2018, Section 5.6) [were / were not] weak nationally. Mid-2017 is when Sentinel-2B began operations, so the 2017 season has fewer observations everywhere. The 2018 drought was regional, so its national effect should be uneven across states [confirm with Table 6].

Crop sequences give a check on classification that uses no reference data. Canola is normally grown as a break crop between cereals, so a plausible map should rarely show canola in consecutive seasons on the same paddock. The mapped rate of XXX% [is / is not] consistent with that practice. Note this means a high rate of repeated canola would point to classification error rather than agronomy.

The 2025 season has no ABS or final ABARES figures. Its map is therefore unscored, and it should be used with the error rates measured on 2022-2024 rather than as a validated result.
