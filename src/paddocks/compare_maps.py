...
IS_CLS = "(abstain_reason = '' OR abstain_reason IS NULL)"


def national(path, layer):
...
def read_box(path, layer, box, pad):
    x0, y0, x1, y1 = box
    g = gpd.read_file(path, layer=layer, bbox=(x0 - pad, y0 - pad, x1 + pad, y1 + pad))
    g["cls"] = g.pred.where(g.abstain_reason.fillna("").eq(""), None)
    return g


def windows(a, b, aois, n, seed, pad=600.0):
    ...
    for stub, box in boxes:
        ...
        A = read_box(a["path"], a["layer"], box, pad)
        B = read_box(b["path"], b["layer"], bx, pad)
        if B.crs != A.crs:
            B = B.to_crs(A.crs)
        win = shapely.box(*box)
...
def main():
    ...
    args = ap.parse_args()
    a = national(args.a, args.a_layer); a["layer"] = args.a_layer
    b = national(args.b, args.b_layer); b["layer"] = args.b_layer
    print("national totals done", flush=True)
    W = windows(a, b, args.aois, args.n_windows, args.seed)
